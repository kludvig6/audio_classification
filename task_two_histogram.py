...
class TaskThreeHistogram:
    def __init__(self):
        self.track_table = AudioTrackTable("GenreClassData_30s.txt")
        self.metal_songs = self.track_table.get_specific_genre("metal")
        self.disco_songs = self.track_table.get_specific_genre("disco")
        self.blues_songs = self.track_table.get_specific_genre("blues")
        self.rock_songs = self.track_table.get_specific_genre("rock")
    ...
    def create_overlapping_histogram(self, feature):
        ...
        metal = [song.features[feature.value] for song in self.metal_songs]
        disco = [song.features[feature.value] for song in self.disco_songs]
        blues = [song.features[feature.value] for song in self.blues_songs]
        rock = [song.features[feature.value] for song in self.rock_songs]
        
        plt.hist(metal, color="blue", bins=32, label="Metal")
        plt.hist(disco, color="green", bins=32, label="Disco")
        plt.hist(blues, color="red", bins=32, label="Blues")
        plt.hist(rock, color="purple", bins=32, label="Rock")
